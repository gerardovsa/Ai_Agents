...
- `archive/timelines_and_reports/` (1,196 added June 12, 2026 — row 51) and `archive/analysis_reports/` (29 added June 12, 2026 — row 51) — 1,225 root `.md` files extracted in bulk. Includes the 433 file "Suspicious" bucket (mostly topic-prefixed one-shots: `AI_AGENTS.md`, `ARCHITECTURE.md`, `THREAD_SYSTEM.md`, `VECTOR_DATABASE.md`, `SYNERGY_COLLABORATION.md`, `AUTOMATION_WORKFLOWS.md`, `TRANSCRIPTION.md`, `MODULES.md`, `QUOTE_CALCULATOR.md`, `INHOUSE_PRINT.md`, `INHOUSE_KANBAN.md`, `PROMPT_CATALOGUE.md`, …) — all 433 had **0 inbound references from any active production doc** (CLAUDE.md, README*, `.github/copilot-instructions.md`, `.github/ORG_CREDENTIALS_*.md`, `.github/VECTOR_DB_*.md`). 14 refs from `.github/AI_DOCUMENTATION_CONSOLIDATION_INSTRUCTIONS.md` (now `archive/documentation/AI_DOCUMENTATION_CONSOLIDATION_INSTRUCTIONS_jan2026.md` per F7) are themselves a meta-instruction, and 8 refs from `.github/prompts/System_Architexture.prompt.md` are a generator prompt. **Only 11 root `.md` files survive:** 7 Contract (untouchable: `README.md`, `README_V11.md`, `CLAUDE.md`, `ARCHIVE_CLEANUP_PLAN.md`, `ARCHIVE_CLEANUP_SUMMARY_NOV30.md`, `ARCHIVE_CLEANUP_TRACKER.md`, `ARCHIVE_SAFETY_VERIFICATION.md`) + 4 Runbook (`CUSTOMER_REACTIVATION_DEPLOYMENT.md`, `MARKDOWN_EXTRACTION_DEPLOYMENT.md`, `PERFORMANCE_OPTIMIZATION_DEPLOYMENT.md`, `START_HERE.md`). See `archive/timelines_and_reports/DEPRECATION_NOTICE_ROW_51.md`. Recovery: `git log --diff-filter=R -- archive/timelines_and_reports/ | grep -v DEPRECATION_NOTICE_ROW_51` (1,196 files) and `git log --diff-filter=R -- archive/analysis_reports/` (29 files).
- `archive/timelines_and_reports/` (102 added June 12, 2026 — row 52) — 102 root non-`.md` files extracted in 4 batches of ≤30. Audit set: 23 .ps1 (one-shot fix scripts: APPLY_*, DIAGNOSE_*, FIX_*, START_WITH_SUPABASE, SUPABASE_SETUP_*, add_*, apply_*, batch_*, check_v10_*, comment_*_font_weights (4), fix_schema_*, fix_ui_paths (2), sync-to-valorai, test_email_*, track_script_balance, verify_ui_paths), 21 .js (one-shot tests/debuggers: CHAT_SIDEBAR_*_TEST, FIX_MISSING_BUTTONS, MASTER_PRIME_DEBUGGER, MESSAGE_TIMESTAMP_UI_*, PRIME_ROOT_CAUSE_FINDER, TEMP_OLD_*, TEMP_V9_*, TEST_PRIME_*, check_*, console_inspector — **NOT archived**, it's `package.json` "main" — debug_*, diagnostic_*, force_reload_*, test_*, watch_dom_mutations), 16 .html (test pages: SETTINGS_BUTTON_INTEGRATION, TEST_SVG_*, TEST_THREAD_*, calculator_test_dashboard — **NOT archived**, opened by main SPA — diagnose_kanban_dashboard, synergy_ui_test, test-ai-*, test_comm_*, test_frontend_imports, test_modern_framework_loading, test_vector_database_v4), 31 .txt (one-shot output/demo/ref cards: ADAPTIVE_INVESTIGATION_FLOWCHART, BISTART_current, COMMIT_COMMANDS, QUICK_REFERENCE_CARD, RENDER_CREDENTIALS_TEMPLATE, RENDER_V10_ENV_VARS, SEARCH_TERM_DEMONSTRATION, SHIPPING_INTEGRATION_DIAGRAM, SPIRAL_FIXED_FEE_*, THREAD_COPY_OUTPUT_*, complete_test_run, database_data_locations, flask_debug_output, full_test_output, meta_test_output, output, semantic_test_*, synergy_*, temp_check, test_77_queries_output, test_*_output (8), thread_2111_*, requirements_text_extraction), 1 .bat (OPEN_APP.bat, 0 refs but kept as Windows shortcut), 3 .sh (RENDER_DOWNLOAD_UPDATED_DBS, create_threads_table_render, integrate-synergy-realtime), 3 .yaml (dead Render configs: render-docker.yaml, render-v10.yaml, render_cleaned.yaml — all 0 refs; render.yaml kept at root, 14 refs), 2 .css (TEMP_OLD_inhouse-kanban, temp_synergy_styles), 1 .svg (architecture_diagram_content_blocks), 1 .csv (thread_locations_export). All 102 had 0 inbound references from any active production doc. **19 files kept at root** as load-bearing or contract: BISTART.bat, BISTOP.ps1, CHAT.bat, CHATM.bat, CHATM.ps1, OPEN_APP.bat, BISTART.ps1, chat.ps1 (called by CHAT.bat), startup.sh, console_inspector.js (package.json main), calculator_test_dashboard.html (opened by main SPA), render.yaml (14 refs), docker-compose.yml (10 refs), Dockerfile (13 refs), package.json, requirements.txt, runtime.txt, .env.example, config.example.py, get_supabase_credentials.py, synergy_requirements.txt. See `archive/timelines_and_reports/DEPRECATION_NOTICE_ROW_52.md`. Recovery: `git log --diff-filter=R -- archive/timelines_and_reports/ | grep -E "^[a-f0-9]+ " | grep -v DEPRECATION` (the 102 row-52 moves are tagged `chore(cleanup/root-scripts-row52)` in commit messages).
