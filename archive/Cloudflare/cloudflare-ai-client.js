...
class CloudflareAIClient {
    ...
    async generateText(prompt, options = {}) {
        ...
        try {
            ...
            console.log('ext generation complete');
    ...
    async generateEmbeddings(text, model = '@cf/baai/bge-base-en-v1.5') {
        ...
        try {
            const response = await this.makeRequest(`/run/${model}`, 'POST', payload);
            console.log('mbeddings generated');
    ...
    async classifyImage(imageBuffer, model = '@cf/microsoft/resnet-50') {
        ...
        try {
            const response = await this.makeRequest(`/run/${model}`, 'POST', payload);
            console.log('mage classification complete');
            return response.result;
        } catch (error) {
    ...
    async listModels() {
        ...
        try {
            ...
            console.log('odels list retrieved');
            return response.result;
        } catch (error) {
    ...
    async verifyToken() {
        ...
        try {
            ...
            console.log('oken is valid - Workers AI is accessible');
            return {
                status: 'Active',
                verified: true,
