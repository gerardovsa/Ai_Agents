...
# Import database connector from inhouse-print module
from db_connector import InHousePrintDB

# Import calculator from implementations folder
...
class ToolUseAgent:
    ...
    def __init__(self, config_path: str, log_callback: Optional[callable] = None):
        ...
        config_full_path = os.path.join(quote_calc_path, '..', '..', config_path)
        ...
        # Initialize stock database tools if available
        if StockDatabaseTools:
            self.stock_tools = StockDatabaseTools()
            # Path to stocks folder (contains stock_data.db)
            self.stocks_path = os.path.join(quote_calc_path, 'stocks')
        else:
            self.stock_tools = None
            self.stocks_path = None
