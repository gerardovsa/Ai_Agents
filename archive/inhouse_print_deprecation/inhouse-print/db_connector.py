        ...
        if self.config is None:
            if config_path is None:
                ...
                search_paths = [
                    ...
                    # From inhouse-print: .. = modules_external/, then quote-calculator/config/
                    os.path.join(current_dir, "..", "quote-calculator", "config", "database-config.json"),
                    # Location 2: Project root config (5 levels up from inhouse-print)
                    os.path.join(current_dir, "..", "..", "..", "..", "config", "database-config.json"),
                    # Location 3: Legacy In_House_SQL project config
