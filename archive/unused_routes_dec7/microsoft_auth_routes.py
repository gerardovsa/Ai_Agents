...
ALL CURSOR MANAGEMENT ISSUES FIXED
✅ Proper cursor/connection initialization
✅ Guaranteed cleanup via finally blocks
✅ Correct close order (cursor before connection)
✅ No unreachable code
...
def microsoft_login():
    ...
    NOTE: No database operations, no cursor management needed
    """
...
def microsoft_callback():
    ...
    ✅ FIXED: Proper cursor management with guaranteed cleanup
    """
    
    # ✅ STEP 1: Validate input BEFORE creating any database resources
    ...
    # ✅ STEP 2: Initialize cursor/connection BEFORE try block
    cursor = None
    conn = None
    ...
    try:
        ...
        # ✅ STEP 4: Close resources BEFORE processing results
        cursor.close()
        cursor = None
        conn.close()
        conn = None
        
        # Clear session (safe - no database resources open)
        session.pop('microsoft_auth_user_id', None)
        session.pop('microsoft_redirect_uri', None)
        
        # ✅ STEP 5: Return response AFTER cleanup
        return jsonify({
            'success': True,
...
def refresh_microsoft_token():
    ...
    ✅ FIXED: Proper management of TWO separate cursor/connection pairs
    """
    
    # ✅ STEP 1: Validate input BEFORE creating resources
    ...
    # ✅ STEP 2: Initialize BOTH cursor/connection pairs
    cursor = None
    conn = None
    cursor2 = None
    conn2 = None
    ...
    try:
        # ✅ STEP 3: First database operation - get stored credentials
        conn = get_db_connection()
        cursor = conn.cursor()
        ...
        # ✅ Close first cursor/connection BEFORE external API call
        cursor.close()
        cursor = None
        conn.close()
        conn = None
        ...
        # ✅ STEP 4: External API call (no database resources open)
        token_data = {
            'client_id': MICROSOFT_CLIENT_ID,
            'client_secret': MICROSOFT_CLIENT_SECRET,
        ...
        # ✅ STEP 5: Second database operation - update tokens
        ...
        # ✅ Close second cursor/connection BEFORE returning
        cursor2.close()
        cursor2 = None
        conn2.close()
        conn2 = None
        
        # ✅ STEP 6: Return response AFTER cleanup
        return jsonify({
            'success': True,
        ...
        # ✅ STEP 7: Guaranteed cleanup of BOTH cursor/connection pairs
        if cursor:
            try:
                cursor.close()
            except:
...
def microsoft_auth_status():
    ...
    try:
        ...
        # ✅ STEP 5: Process results AFTER database cleanup
...
def get_microsoft_access_token(user_id):
    ...
    Returns:
        dict: {'success': bool, 'access_token': str} or {'success': bool, 'error': str}
    """
    
    # ✅ STEP 1: Initialize BOTH cursor/connection pairs
    cursor = None
    conn = None
    cursor2 = None
    conn2 = None
    ...
    try:
        # ✅ STEP 2: First database operation - get stored credentials
        conn = get_db_connection()
        cursor = conn.cursor()
        ...
        # ✅ Close first cursor/connection BEFORE checking result
        cursor.close()
        cursor = None
        conn.close()
        conn = None
        ...
        if datetime.utcnow() >= (token_expiry - timedelta(minutes=5)):
            ...
            # External API call (no database resources open)
            token_data = {
                'client_id': MICROSOFT_CLIENT_ID,
                'client_secret': MICROSOFT_CLIENT_SECRET,
                'refresh_token': refresh_token,
            ...
            # ✅ STEP 4: Second database operation - update tokens
            conn2 = get_db_connection()
            cursor2 = conn2.cursor()
        ...
        return {'success': False, 'error': f'Token retrieval failed: {str(e)}'}
    
    finally:
        # ✅ STEP 5: Guaranteed cleanup of BOTH cursor/connection pairs
        if cursor:
            try:
                cursor.close()
            except:
