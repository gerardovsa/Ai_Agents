...
| 50 | **Top-level `*.py` one-shot scripts cleanup** (was F5) | code | All 430 .py at the repo root. Apply row-40 heuristic aggressively: not imported anywhere, not in `.vscode/tasks.json`, not in `.github/workflows/`, last modified >6 months. | claude-row50-2026-06-12 | 2026-06-12 | IN PROGRESS | cleanup/root-one-shot-cleanup | Branch created off `cleanup/top-tests-cleanup`. Per user: "delete any that are old or not used". Heuristic: last modified >6 months (before 2025-12-12) AND zero importers. **Hard rule: never `rm`, always `git mv`** to `archive/root_one_shot_row50/` (batches of ≤30). **Touched file path note**: this is the "row 51" the user referenced; tracker uses row 50 because the 49 + 1 = 50 sequential. |
