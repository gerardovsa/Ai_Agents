...
| 51 | **Top-level `*.md` doc cleanup** (was F4) | docs | All 1,236 `.md` at the repo root. Per CLAUDE.md §16.5 workflow: inventory → classify (7 buckets per §16.3) → verify (sample 2-3 facts per Canonical) → propose → execute. Batches of ≤30 per the per-feature prompt §6 (1,236 / 30 ≈ 42 commits minimum). 4 sub-categories expected: Contract (untouchable), Runbook (keep/promote), Canonical (keep + cross-link), Historical/Scratch/Debug (move to `archive/documentation` or `archive/timelines_and_reports` or `archive/analysis_reports`). Expected 5+ sub-batches per F4's note. | claude-row51-2026-06-12 | 2026-06-12 | IN PROGRESS | cleanup/root-md-cleanup | Branched off `cleanup/root-one-shot-cleanup` to preserve chain. Per user: "continue with the rroot md files" (June 12, 2026). Heuristic from F4 + §16.3: filename pattern + size + age + first-line title. **Hard rules** (per-feature prompt): never `rm`, never `delete` an old `.md`, never break inbound links silently, never touch `README.md` / `README_V11.md` / `LICENSE` / `requirements.txt` / `package.json` / `runtime.txt` / `.env.example` / `.env.master`. **F4 tombstone** stays below until row 51 is DONE. |
