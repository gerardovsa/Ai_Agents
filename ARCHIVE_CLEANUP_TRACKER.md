...
| F5 | discovered post-row-42 | **430 root `.py` files** need audit (CODE GAP — row 40 only covers `AI_infrastructure/`, not root) | code | Inventory: 47 <1KB (likely throwaway), 220 1-5KB, 153 5-20KB, 10 >20KB. **132 match one-shot patterns** (`verify_` 21, `test_` ?, `validate_` ?, `update_` ?, `fix_` ?, `check_` ?, `find_` ?, `analyze_` ?, `scan_` ?, `audit_` ?, `add_` ?, `apply_` ?, `backfill_` 2, `bulk_` 1, etc.). Heuristic from row 40 spec applies: not imported anywhere, not in `.vscode/tasks.json`, not in `.github/workflows/`, last modified >6 months. **This needs its own dedicated row** (proposed row 51). Examples already flagged: `bulk_fix_schema_prefix.py` (already moved in row 44, wait — no, **it is still at the root**; the 9.5K top-level `test_inhouse_calculator_fix.py` flagged in row 42 is here too). |
