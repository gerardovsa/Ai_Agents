...
| F4 | discovered post-row-42 | **1,236 root `.md` files** need audit (DOC GAP — not covered by any of the 49 rows) | docs | Inventory: 4 <1KB, 69 1-5KB, **958 are 5-20KB** (the bulk — typical fix-log/analysis size), 203 20-100KB, 2 >100KB. **The previous biggest doc cleanup (row 47) was 271 file ops; this is ~4.5x larger.** Playbook: CLAUDE.md §16.5 (inventory → classify → verify → propose → execute; batches of ≤30). **Sub-categories expected per §16.3**: Contract (keep — `README.md`, `README_V11.md`, `LICENSE` are untouchable), Runbook (promote or keep at root), Canonical (keep + cross-link from CLAUDE.md), Historical (move to `archive/documentation/`), Scratch (move to `archive/timelines_and_reports/`), Deprecated (bundle + DEPRECATION_NOTICE.md), Debug (move to `archive/analysis_reports/`). **This needs its own dedicated row** (proposed row 50). Cannot be done as a single batch — must split into 5+ sub-batches. |
| F5 | discovered post-row-42 | **430 root `.py` files** need audit (PROMOTED to row 50 — in progress) | code | ~~Inventory + heuristic~~. **This work is being executed as row 50** on branch `cleanup/root-one-shot-cleanup`. F5 stays here as a tombstone until row 50 is DONE. |
