...
| 43 | Top-level "integration" directories | meta | `Woocommerce/`, `Cloudflare/`, `Render_backend/`, `Supabase/`, `Microsoft_365_Connection/`, `google_workspace/`, `tslot_bed_frame_docs/`, `temp_v9_comparison/`. For each: is anything imported from here? Is the content archived elsewhere? | | | TODO | | |
...
| 45 | Top-level archive directories audit | meta | `ARCHIVE_OCT30_2025/`, `archive/`, `_ARCHIVED_NOV25/`, `UI/ARCHIVE_OLD_UI_20251030_223356/`, `UI/components_ARCHIVED/`. For each: confirm contents are truly historical; consolidate where appropriate. | claude-row45-2026-06-11 | 2026-06-11 | DONE | cleanup/top-archive-audit | **Audit + execution.** `_ARCHIVED_NOV25/` does not exist. Audited 4 dirs: `archive/` (13 subdirs, 636 files, ~40M), `ARCHIVE_OCT30_2025/` (3 subdirs, 14 files, 140K), `UI/ARCHIVE_OLD_UI_20251030_223356/` (12 files, 1.8M), `UI/components_ARCHIVED/` (5 files, 97K). All historical — no code imports from any. 2 JSX files in `UI/components_ARCHIVED/` are 100% dead (vanilla-JS SPA can't compile JSX). Overlap with `docs/archive/` (row 47 result): only 3 basenames match — keep both trees. Findings doc: `docs/agents/ROW_45_TOP_LEVEL_ARCHIVE_AUDIT.md`. **Followup executed (4 commits, 30 file ops)**: 2 UI-level archive dirs moved to `archive/ui_archives_20251030/` (12 files) and `archive/ui_components_archived/` (5 files); 2 exact-dup/'_ copy' HTMLs prefixed `_DUPLICATE_`; 9 regenerable HTML dashboards + 2 git-log TXT dumps prefixed `_ARCHIVED_`; 2 DEPRECATION_NOTICE.md added; CLAUDE.md line 139 updated.
...
2026-06-11 | top-archive-audit | cleanup/top-archive-audit | local-branch-3fc95392 | Row 45 done: read-only audit + execution. 4 dirs audited: archive/ (13 subdirs, 636 files, ~40M), ARCHIVE_OCT30_2025/ (3 subdirs, 14 files, 140K), UI/ARCHIVE_OLD_UI_20251030_223356/ (12 files, 1.8M), UI/components_ARCHIVED/ (5 files, 97K). _ARCHIVED_NOV25/ doesn't exist. 2 JSX files in UI/components_ARCHIVED/ are 100% dead code (vanilla-JS SPA). Overlap with docs/archive/ (row 47): only 3 basenames match — keep both trees. **Followup: 4 commits, 30 file ops.** 2 UI-level archive dirs moved (UI/ARCHIVE_OLD_UI_20251030_223356/ → archive/ui_archives_20251030/; UI/components_ARCHIVED/ → archive/ui_components_archived/). 2 HTMLs prefixed _DUPLICATE_ (md5-verified). 9 regenerable HTML dashboards + 2 git-log TXT dumps prefixed _ARCHIVED_. 2 DEPRECATION_NOTICE.md added. CLAUDE.md line 139 updated. Findings doc: docs/agents/ROW_45_TOP_LEVEL_ARCHIVE_AUDIT.md. | DONE
```
