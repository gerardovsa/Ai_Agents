        ...
        while (tool_uses and current_response.get('stop_reason') == 'tool_use' and
            ...
            for tool_use in tool_uses:
                ...
                try:
                    ...
                    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
                    # Lazy-mode (2026-07-27): materialize FIRST so a legitimate
                    # not-yet-loaded tool passes the gate. materialize_tool()
                    # is idempotent — already-loaded tools short-circuit.
                    if not registry.materialize_tool(tool_name) or not registry.is_tool_allowed(tool_name):
                        # Build a small "did you mean" hint to help the
                        # model recover without burning another round.
                        # Use the SCHEMA INDEX (not just loaded tools) so the
                        # hint can suggest not-yet-materialized candidates —
                        # critical in lazy mode where only 8 meta-tools are
                        # loaded at boot and the AI is being told its guess
                        # was wrong.
                        try:
                            same_prefix = sorted({
                                n for n in registry._schema_index.keys()
                                if isinstance(n, str)
