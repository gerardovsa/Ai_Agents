...
def execute_streaming_request(
    ...
    try:
        ...
        # CRITICAL FIX: Check if last message has orphaned tool_use blocks
        if messages and messages[-1].get('role') == 'assistant':
            last_content = messages[-1].get('content', [])
            if isinstance(last_content, list):
                tool_use_ids = [b.get('id') for b in last_content if b.get('type') == 'tool_use']
                
                if tool_use_ids:
                    print(f"{log_prefix} ⚠️ WARNING: Last assistant message has {len(tool_use_ids)} tool_use blocks without tool_result!")
                    print(f"{log_prefix} Tool IDs: {tool_use_ids}")
                    print(f"{log_prefix} 🔧 Removing problematic assistant message to prevent API error")
                    messages.pop()  # Remove the assistant message with orphaned tool_use
