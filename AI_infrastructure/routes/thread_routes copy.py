...
def list_available_agents():
    ...
    try:
        ...
        with get_database_connection('sessions') as conn:
            ...
            if user_id:
                sql += " AND user_id = %s"
                cursor.execute(sql + " GROUP BY location ORDER BY location", (user_id,))
            else:
                cursor.execute(sql + " GROUP BY location ORDER BY location")
            
            agent_stats = cursor.fetchall()
            cursor.close()
        
        # Build agent list from stats
        for row in agent_stats:
            ...
            # Extract agent number from location (e.g., 'agent-1' -> 1)
                ...
                try:
                    ...
                    if 1 <= agent_num <= 26:
                        ...
                        agents.append({
                            'id': location,
                            'name': agent_name,
                            'description': f"Active threads: {thread_count}",
        ...
        cursor.close()  # ✅ AUTO-FIXED: Close cursor before return/commit
...
def list_threads():
    ...
    try:
        ...
        with get_database_connection('sessions') as conn:
            ...
            cursor.close()  # ✅ CRITICAL: Close cursor before returning connection to pool
    ...
    try:
        ...
        with get_database_connection('sessions') as conn:
            ...
            try:
                ...
                cursor.close()  # ✅ AUTO-FIXED: Close cursor before return/commit
                if cursor:
                    cursor.close()  # ✅ GUARANTEED: Close cursor even on exception
    ...
    try:
        ...
        with get_database_connection('sessions') as conn:
            ...
            try:
                ...
                if cursor:
                    cursor.close()  # ✅ GUARANTEED CURSOR CLEANUP
