...
class BaseModule {
    constructor(moduleId, modulePath = null) {
        ...
        // ✅ Provide standard backend URL for API calls
    ...
    async initialize() {
        ...
        // Load manifest FIRST (so we know main_tab_id)
        await this.loadManifest();

        // Get container using main_tab_id from manifest (or fallback to moduleId)
        const tabId = this.manifest?.main_tab_id || this.moduleId;
        this.container = document.getElementById(`tab-${tabId}`);
        if (!this.container) {
            throw new Error(`Container not found for module: ${this.moduleId} (looking for #tab-${tabId})`);
        }
        console.log(`✅ Found container: #tab-${tabId}`);
