...
export default {
    ...
    initializeKanbanBoard() {
        ...
        container.innerHTML = `
            ...
            <div style="margin: 15px 20px; padding: 12px; background: #0d1117; border: 1px solid #30363d; border-radius: 6px; display: flex; align-items: center; gap: 20px; flex-wrap: wrap;">
                ...
                <div style="margin-left: auto; display: flex; gap: 10px;">
                    <button onclick="window.currentKanbanModule.clearAllMutedCards()" class="btn-sm" style="padding: 4px 10px; background: #0d1117; border: 1px solid #da3633; color: #da3633; border-radius: 4px; cursor: pointer; font-size: 0.85em;">
                        Clear Muted (<span class="muted-cards-count">0</span>)
                    </button>
                    <button onclick="window.currentKanbanModule.showMutedCardsOnly()" class="btn-sm" style="padding: 4px 10px; background: #0d1117; border: 1px solid #58a6ff; color: #58a6ff; border-radius: 4px; cursor: pointer; font-size: 0.85em;">
                        Show Muted
                    </button>
                    <button onclick="window.currentKanbanModule.exportMutedCards()" class="btn-sm" style="padding: 4px 10px; background: #0d1117; border: 1px solid #238636; color: #238636; border-radius: 4px; cursor: pointer; font-size: 0.85em;">
                        Export
                    </button>
                </div>
            </div>
            ...
            <div class="inhouse-kanban-metrics" id="kanban-metrics" style="margin: 15px 20px; display: flex; gap: 15px;"></div>
            ...
            <div style="margin: 20px; padding: 20px; background: #0d1117; border: 1px solid #30363d; border-radius: 6px;">
                <h3 style="margin: 0 0 15px 0; color: #f0f6fc; font-size: 16px; font-weight: 600; display: flex; align-items: center; gap: 8px;">
                    <i class="fas fa-palette"></i>
        ...
        this.ui.kanbanBoard = container.querySelector('#kanban-board');
        this.ui.workboardSelector = container.querySelector('#workboard-selector');
        this.ui.metricsContainer = container.querySelector('#kanban-metrics');
    },
    ...
    async loadStages() {
        try {
            ...
            const data = await response.json();
            this.state.stages = Array.isArray(data) ? data : data.stages || [];
    ...
    renderMetrics() {
        ...
        this.ui.metricsContainer.innerHTML = `
            <div class="metric-card" style="flex: 1; padding: 15px; background: linear-gradient(135deg, #1e3a8a 0%, #1e40af 100%); border-radius: 8px; text-align: center;">
                <div style="font-size: 32px; font-weight: 700; color: #ffffff; margin-bottom: 5px;">${totalJobs}</div>
                <div style="font-size: 13px; color: rgba(255,255,255,0.8); font-weight: 600;">TOTAL JOBS</div>
            </div>
            <div class="metric-card" style="flex: 1; padding: 15px; background: linear-gradient(135deg, #dc2626 0%, #ef4444 100%); border-radius: 8px; text-align: center;">
                <div style="font-size: 32px; font-weight: 700; color: #ffffff; margin-bottom: 5px;">${criticalJobs}</div>
                <div style="font-size: 13px; color: rgba(255,255,255,0.8); font-weight: 600;">CRITICAL</div>
            </div>
            <div class="metric-card" style="flex: 1; padding: 15px; background: linear-gradient(135deg, #ea580c 0%, #f97316 100%); border-radius: 8px; text-align: center;">
                <div style="font-size: 32px; font-weight: 700; color: #ffffff; margin-bottom: 5px;">${overdueJobs}</div>
                <div style="font-size: 13px; color: rgba(255,255,255,0.8); font-weight: 600;">OVERDUE</div>
            </div>
        `;
    },
    ...
    getCardBorderStyle(job, muteSettings = {}) {
        if (!this.colorToggles.showBorderColors || muteSettings.muteBorder) {
    ...
    getCardBackgroundStyle(job, muteSettings = {}) {
        if (!this.colorToggles.showBackgroundColors || muteSettings.muteBackground) {
            return '#161b22';
        }
    ...
    getCardBanner(job, muteSettings = {}) {
        if (!this.colorToggles.showBannerColors || muteSettings.muteBanner) {
            return '';
        }
    ...
    getMuteSettings(jobId) {
        return this.mutedCards.get(String(jobId)) || null;
    },
