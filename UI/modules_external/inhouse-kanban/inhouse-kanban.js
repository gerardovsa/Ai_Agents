...
C:\Users\gpoli\GIT\AI_agents\UI\modules_external\inhouse-kanban\inhouse-kanban.js
...
console.log('🔷 InHouse Kanban Module Loading - VERSION 3.1 - Uses Global BaseModule');
...
if (typeof BaseModule === 'undefined') {
    console.error('❌ [InHouse Kanban] CRITICAL: BaseModule not found! Module cannot load.');
...
console.log('✅ [InHouse Kanban] BaseModule found globally, extending it...');
...
class InhouseKanbanModule extends BaseModule {
    constructor(moduleId) {
        ...
        this.backendUrl = this.API_BASE_URL; // ✅ FIX: Set backendUrl for API calls
    ...
    async initialize() {
        ...
        try {
            ...
            if (!this.jobs || this.jobs.length === 0) {
                ...
                // CRITICAL FIX: Render the board after data loads
        ...
        // Initialize logger (after KanbanLogger script loads)
        this.initializeLogger();

        this.logger.success('InHouse Print Production Workflow module ready', {
            jobs: this.jobs?.length || 0,
     ...
     * V3.0 Compliance: Replaces excessive console.log usage
     */
    initializeLogger() {
        if (window.KanbanLogger) {
            this.logger = new window.KanbanLogger({
                enabled: true,
                level: 'info', // debug, info, warn, error
                category: 'inhouse-kanban',
                maxLogs: 1000
            });
            this.logger.info('Logger initialized');
        } else {
            // Fallback to console if logger not loaded
            console.warn('⚠️ KanbanLogger not available, using console fallback');
            this.logger = {
                ...
                loading: (...args) => console.log(...args),
                data: (...args) => console.log(...args),
                network: (...args) => console.log(...args),
                ui: (...args) => console.log(...args),
    ...
    /**
     * Render method for V3.0 compliance
     * Main rendering entry point for the Kanban board
     */
    render() {
        ...
        try {
            ...
            if (!container) {
                this.logger?.error('Failed to get container for Kanban board');
                return;
            }

            // Render the complete Kanban board UI
            this.renderBoardUI(container);

            // Setup event listeners
            this.setupEventListeners();

            // Render the data (jobs on the board)
            if (this.jobs && this.jobs.length > 0) {
                this.renderKanbanBoard();
            } else {
                this.logger?.warn('No jobs data available for rendering');
                this.showEmptyState(container);
            }

            this.logger?.success('Kanban board rendered successfully', {
                jobs: this.jobs?.length || 0
            });
        } catch (error) {
            this.logger?.error('Failed to render Kanban board', error);
            this.showErrorState(error);
        }
    }

    /**
     * Render the board UI structure (called by render and initializeKanbanBoard)
     */
    renderBoardUI(container) {
        const primaryColor = this.manifest?.colors?.primary || '#00509E';

        // This HTML generation is extracted from initializeKanbanBoard
        // to support both initialization and re-rendering
        container.innerHTML = this.generateBoardHTML(primaryColor);
    }

    /**
     * Generate board HTML structure
     */
    generateBoardHTML(primaryColor) {
        return `
            <!-- Module Header -->
            ...
            ${this.generateFiltersHTML(primaryColor)}
            ${this.generateColorToggleHTML()}
            ${this.generateMetricsHTML()}
            ${this.generateLegendHTML()}
            
            <div class="kanban-board" id="kanban-board" style="min-height: 600px;">
                <!-- Board columns will be dynamically inserted here -->
            </div>
        `;
    }

    /**
     * Generate filters bar HTML
     */
    generateFiltersHTML(primaryColor) {
        return `
    ...
    /**
     * Generate color toggle controls HTML
     */
    generateColorToggleHTML() {
        return `
            <div style="margin: 15px 20px; padding: 12px; background: #0d1117; border: 1px solid #30363d; border-radius: 6px; display: flex; align-items: center; gap: 20px; flex-wrap: wrap;">
                ...
                <button id="toggle-border-colors" class="color-toggle-btn active" 
                        ...
                        title="Toggle colored borders">
                    <i class="fas fa-square-full"></i> Borders
                </button>
                <button id="toggle-background-colors" class="color-toggle-btn active" 
                        ...
                        title="Toggle colored backgrounds">
                    <i class="fas fa-fill"></i> Backgrounds
                </button>
                <button id="toggle-text-colors" class="color-toggle-btn active" 
                        onclick="window.currentKanbanModule.toggleColorSetting('showTextColors')" 
                        title="Toggle colored text">
    ...
    /**
     * Generate metrics HTML (placeholder)
     */
    generateMetricsHTML() {
        return `<div id="kanban-metrics-container"></div>`;
    }

    /**
     * Generate legend HTML (placeholder)
     */
    generateLegendHTML() {
        return `<div id="kanban-legend-container"></div>`;
    }
    ...
    showEmptyState(container) {
        ...
        emptyState.style.cssText = 'padding: 60px 20px; text-align: center; color: #8b949e;';
        emptyState.innerHTML = `
            <i class="fas fa-inbox" style="font-size: 64px; opacity: 0.3; margin-bottom: 20px;"></i>
            <h3 style="color: #f0f6fc; margin-bottom: 10px;">No Jobs Found</h3>
            <p>No jobs match the current filters. Try adjusting your filters or refresh the data.</p>
        `;

        const boardContainer = container.querySelector('#kanban-board-container');
        if (boardContainer) {
    ...
    showErrorState(error) {
        this.logger?.error('Showing error state', error);
        const container = this.getSubTabContainer('kanban-board');
        if (container) {
            container.innerHTML = `
                <div style="padding: 60px 20px; text-align: center; color: #8b949e;">
                    ...
                    <button onclick="window.currentKanbanModule.render()" style="margin-top: 20px; padding: 10px 20px; background: #238636; color: white; border: none; border-radius: 6px; cursor: pointer;">
    ...
    injectCriticalStyles() {
        ...
        style.textContent = `
            ...
            .sub-tab-btn.active {
                color: #FFFFFF !important;
                border-bottom-color: var(--module-primary-color, #00509E) !important;
            }
     ...
     * @param {string} tabName - Name of the sub-tab ('kanban-board', 'workboard', 'analytics', or null for main)
     * @returns {HTMLElement} The appropriate sub-tab container or main container
     */
    getSubTabContainer(tabName) {
        // If tabName is provided, return specific sub-tab container
        if (tabName) {
            // Normalize tab names: 'kanban-board' or 'workboard' → 'workboard'
            const normalizedTabName = (tabName === 'kanban-board' || tabName === 'workboard') ? 'workboard' : tabName;

            // Get the specific sub-tab container
            const subTabContainer = document.getElementById(`${this.manifest.id}-subtab-${normalizedTabName}`);

            if (subTabContainer) {
                console.log(`[InHouseKanban] Returning sub-tab container: #${this.manifest.id}-subtab-${normalizedTabName}`);
                return subTabContainer;
            } else {
                console.warn(`[InHouseKanban] Sub-tab container #${this.manifest.id}-subtab-${normalizedTabName} not found! Falling back to main container.`);
            }
        }

        // No tabName or sub-tab container not found → return main container
     ...
     */
    initializeSubTabs() {
        this.activeSubTab = 'workboard';  // Default tab
        this.createSubTabNavigation();
        this.initializeKanbanBoard();
        this.initializeAnalytics();
        this.switchSubTab('workboard');  // Show default tab
    }
    ...
    switchSubTab(tabName) {
        console.log(`🔄 Switching to sub-tab: ${tabName}`);
        ...
        if (tabName === 'workboard') {
            workboardContent.style.display = 'flex';
            analyticsContent.style.display = 'none';
        } else if (tabName === 'analytics') {
            workboardContent.style.display = 'none';
    ...
    initializeKanbanBoard() {
        this.logger?.info('Initializing Kanban board structure...');
        ...
        this.loadInitialData().then(() => {
            this.logger?.info('Initial data loaded, rendering components...');
            ...
            this.logger?.success('Kanban board fully rendered');
        }).catch(error => {
    ...
    renderStageColumn(stage, jobs) {
        ...
        return `
            <div class="inhouse-kanban-column" data-stage-id="${stage.StageID}" style="border-left: 4px solid ${stageColor};">
                <div class="inhouse-kanban-column-header" style="border-bottom-color: ${stageColor}; background: linear-gradient(135deg, ${stageColor}20 0%, ${stageColor}10 100%);">
    ...
    renderJobCard(job) {
        ...
        // Get description - use ProductionNotes or ShortJobDesc as fallback
        const description = job.ProductionNotes || job.ShortJobDesc || 'No description available';
        
        // Get product type and quality from correct database fields
        ...
        // Customer tier with color
        const tierInfo = this.getCustomerTierInfo(job.CustomerOrderCount);
        
        // Finishing options icons
        ...
        const businessBadge = job.InvoicingBusiness ? `<span class="inhouse-kanban-business-badge" title="${this.escapeHtml(job.InvoicingBusiness)}"><i class="fas fa-building"></i> ${this.escapeHtml(job.InvoicingBusiness.substring(0, 10))}</span>` : '';
        ...
        return `
            <div class="inhouse-kanban-card${cardStyles.pulseClass}" 
                ...
                <div class="inhouse-kanban-card-header">
                    <div class="inhouse-kanban-card-priority" style="color: ${priorityColor};">
                        <i class="fas ${priorityInfo.icon}"></i>
                    </div>
                    <span class="inhouse-kanban-tier-badge" style="background-color: ${tierInfo.color};" title="${tierInfo.label}">
                        <i class="fas ${tierInfo.icon}"></i>
                    </span>
                </div>
                
                <div class="inhouse-kanban-card-meta">
                    <div class="inhouse-kanban-card-client-header">
                    ...
                    ${job.QTY ? `<div class="inhouse-kanban-card-quantity"><i class="fas fa-boxes"></i> Qty: ${job.QTY} ${paperSpecs ? `<span class="inhouse-kanban-paper-specs">${paperSpecs}</span>` : ''}</div>` : ''}
                    ${finishingIcons ? `<div class="inhouse-kanban-finishing-icons">${finishingIcons}</div>` : ''}
                </div>
                ...
                <div class="inhouse-kanban-card-footer">
                    <div class="inhouse-kanban-card-tags">
                        <span class="inhouse-kanban-card-tag"><i class="fas fa-ticket-alt"></i> ${job.TicketID}</span>
                        <span class="inhouse-kanban-card-tag"><i class="fas fa-file-invoice"></i> ${job.OrderID}</span>
                            ...
                            <div class="kanban-content-section">
                                ...
                                <div class="kanban-section-content">
                                    <div class="kanban-detail-grid">
                                        ...
                                        ${job.StitchYes ? `
                                        <div class="kanban-detail-item">
                                            <span class="kanban-detail-label"><i class="fas fa-stitch"></i> Stitching</span>
                                ...
                                <div class="kanban-section-content">
                                    <div class="kanban-detail-grid">
                                        <div class="kanban-detail-item">
                                            ...
                                            <span class="kanban-detail-value">${this.getCustomerTierInfo(job.CustomerOrderCount).label}</span>
                                        </div>
                                        <div class="kanban-detail-item">
    ...
    showMutedCardsOnly() {
        ...
        if (searchInput) {
            ...
            document.querySelectorAll('.inhouse-kanban-card').forEach(card => {
                const ticketId = parseInt(card.dataset.jobId);
...
window.ModuleRegistry['inhouse_print'] = {
    ...
    filterKanbanBoard() {
        ...
        const cards = document.querySelectorAll('.inhouse-kanban-card');
...
class InhouseKanbanSidebar {
    ...
    initializeSidebar() {
        ...
        // Set initial position based on toggle side
        if (this.sidebarElement) {
            const toggleSide = localStorage.getItem('inhouse-kanban-toggle-side') || 'right';
            // Ensure updateSidebarPosition exists; if not, defer until method added
            if (typeof this.updateSidebarPosition === 'function') {
                this.updateSidebarPosition(toggleSide);
            } else {
                // Fallback: apply basic positioning (match SidebarManager standards)
                if (toggleSide === 'right') {
                    this.sidebarElement.style.right = '60px';
                    this.sidebarElement.style.left = 'auto';
                    this.sidebarElement.style.removeProperty('left'); // CRITICAL: Remove left
                    this.sidebarElement.style.transform = 'translateX(calc(100% + 60px))';
                } else {
                    this.sidebarElement.style.left = '60px';
                    this.sidebarElement.style.right = 'auto';
                    this.sidebarElement.style.removeProperty('right'); // CRITICAL: Remove right
                    this.sidebarElement.style.transform = 'translateX(calc(-100% - 60px))';
                }
            }
        }
    ...
    /**
     * Update sidebar position and transform based on side ('left'|'right')
     * Ensures the sidebar slides in from the correct side and sits 60px from top
     * NOTE: SidebarManager controls the transform - we only set positioning props
     */
    updateSidebarPosition(side) {
        if (!this.sidebarElement) return;

        // Normalize
        const toggleSide = (side === 'right') ? 'right' : 'left';

        // Set top offset consistent with SidebarManager
        this.sidebarElement.style.top = '60px';
        this.sidebarElement.style.height = 'calc(100vh - 60px)';

        if (toggleSide === 'right') {
            this.sidebarElement.style.right = '60px';  // 60px from right edge
            this.sidebarElement.style.left = 'auto';   // CRITICAL: Remove left positioning
            this.sidebarElement.style.removeProperty('left'); // Ensure no left style at all
            this.sidebarElement.style.borderLeft = '1px solid var(--border-default)';
            this.sidebarElement.style.borderRight = 'none';
            this.sidebarElement.style.boxShadow = '-4px 0 24px rgba(0, 0, 0, 0.3)';
            // Check for both 'active' and 'expanded' classes (SidebarManager uses 'expanded')
            const isOpen = this.sidebarElement.classList.contains('active') ||
            ...
            this.sidebarElement.style.left = '60px';  // 60px from left edge
            this.sidebarElement.style.right = 'auto';  // CRITICAL: Remove right positioning
            this.sidebarElement.style.removeProperty('right'); // Ensure no right style at all
            this.sidebarElement.style.borderRight = '1px solid var(--border-default)';
        ...
        // Persist side choice
        localStorage.setItem('inhouse-kanban-toggle-side', toggleSide);
    }
    ...
    closeSidebar() {
        // Use SidebarManager framework for proper close behavior
        if (window.SidebarManager) {
            window.SidebarManager.close('inhouse-kanban-sidebar');
        } else {
            // Fallback if SidebarManager not available
            if (!this.sidebarElement) return;
            this.isOpen = false;
            this.sidebarElement.classList.remove('active');
            this.sidebarElement.classList.add('collapsed');
            // Re-apply hidden transform based on current side
            const side = this.sidebarElement.dataset.side || 'right';
            if (side === 'right') {
                this.sidebarElement.style.transform = 'translateX(calc(100% + 60px))';
            } else {
                this.sidebarElement.style.transform = 'translateX(calc(-100% - 60px))';
...
window.ModuleRegistry['inhouse-kanban'] = {
    ...
    init: async () => {
        ...
        try {
            ...
            // NOTE: Sidebar HTML is already loaded by ModuleLoader (via manifest.json html_file)
            // No need to load it again here - ModuleLoader handles HTML injection
            console.log('✅ Sidebar HTML already loaded by ModuleLoader');
            ...
            if (window.SidebarManager) {
                ...
                try {
                    window.SidebarManager.register({
                        ...
                        toggleButtonId: 'inhouse-kanban-floating-toggle',
                        side: 'right',
                        width: '480px',
                        draggable: true,
                        persistent: true,
                        onInit: async () => {
                            console.log('🔧 SidebarManager.onInit: Initializing InHouse Kanban sidebar...');
                            // Sidebar already initialized by InhouseKanbanSidebar class
                            // Just ensure data is loaded and refresh if available
                            if (window.inhouseKanbanSidebar && module.jobs && Array.isArray(module.jobs)) {
                                console.log('🔄 Loading sidebar cards with loaded data...');
                                window.inhouseKanbanSidebar.loadColumnCards();
                            } else {
                                console.log('ℹ️ Sidebar initialized, waiting for data load...');
                            }
                        },
                        onOpen: () => {
                            console.log('✅ SidebarManager.onOpen: Sidebar opened');
                            // Delegate to existing sidebar class methods
                            if (window.inhouseKanbanSidebar) {
                                window.inhouseKanbanSidebar.isOpen = true;
                                if (window.inhouseKanbanSidebar.sidebarElement) {
                                    window.inhouseKanbanSidebar.sidebarElement.classList.add('active');
                                }
                                // Load cards when opening (in case data changed)
                        ...
                        onClose: () => {
                            console.log('✅ SidebarManager.onClose: Sidebar closed');
                            // Delegate to existing sidebar class methods
                            if (window.inhouseKanbanSidebar) {
