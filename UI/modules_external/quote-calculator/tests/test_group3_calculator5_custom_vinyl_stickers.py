"""
Test Group 3 Calculator 5: Custom Vinyl Stickers

Backend Parameters (verified CustomVinylStickers_Shopify_Calculator.py lines 76-79):
- quantity: int (default 100)
- width_mm (or width): Decimal (default 100)
- height_mm (or height): Decimal (default 100)
- finish: str (default "Gloss")
...
def test_custom_vinyl_stickers_with_new_parameters():
    """Test #1: New parameters (all explicit values) - should work"""
    print("\n🧪 TEST 1: Custom Vinyl Stickers with NEW parameters (all explicit)")
    print("=" * 80)
    ...
    assert result["quantity"] == 200, f"Expected quantity=200, got {result.get('quantity')}"
    ...
    print(f"✅ TEST 1 PASSED: Custom Vinyl Stickers quote calculated successfully")
...
def test_custom_vinyl_stickers_with_legacy_parameters():
    """Test #2: Legacy parameters (width, height) - should work with warnings"""
    print("\n🧪 TEST 2: Custom Vinyl Stickers with LEGACY parameters (width, height)")
    print("=" * 80)
...
def test_custom_vinyl_stickers_price_consistency():
...
def test_custom_vinyl_stickers_defaults_validation():
    """Test #4: Validation with None defaults - backend defaults should apply"""
    print("\n🧪 TEST 4: Custom Vinyl Stickers defaults validation (None → backend defaults)")
    print("=" * 80)
    ...
    assert specs["width_mm"] == 100, f"Expected width_mm=100 (backend default), got {specs['width_mm']}"
    
    assert "height_mm" in specs, "Missing height_mm in specifications"
    assert specs["height_mm"] == 100, f"Expected height_mm=100 (backend default), got {specs['height_mm']}"
    
    assert "finish" in specs, "Missing finish in specifications"
    assert specs["finish"] == "Gloss", f"Expected finish='Gloss' (backend default), got '{specs['finish']}'"
    ...
    print(f"   finish: {specs['finish']} (backend default)")
    print(f"   Total Price: ${result['total_price']:.2f}")


if __name__ == "__main__":
    print("\n" + "=" * 80)
    print("GROUP 3 CALCULATOR 5: Custom Vinyl Stickers - 4 Test Suite")
    print("=" * 80)
    
    try:
        test_custom_vinyl_stickers_with_new_parameters()
        test_custom_vinyl_stickers_with_legacy_parameters()
        test_custom_vinyl_stickers_price_consistency()
        test_custom_vinyl_stickers_defaults_validation()
        
        print("\n" + "=" * 80)
        print("🎉 ALL 4 TESTS PASSED FOR CUSTOM VINYL STICKERS")
        print("=" * 80)
        print("✅ Pattern 3.0/3.0 - Decorator ✅, None Defaults ✅, Validation ✅")
        print("✅ Backend Parameters: 4/4 verified (quantity, width_mm, height_mm, finish)")
