...
async function sendChatMessage() {
    ...
    try {
        ...
        if (contentType && contentType.includes('text/event-stream')) {
                            ...
                            try {
                                ...
                                if (jsonStr) {
                                    ...
                                    if (data.type === 'thinking_block' || data.type === 'thinking') {
                                        ...
                                        // Frontend needs to sync so next request includes proper structure
                                        if (data.conversation_history && Array.isArray(data.conversation_history)) {
                                            ...
                                            console.log('[WARN] No conversation_history in complete event - frontend/backend may be out of sync');
                                        }
            ...
            // CRITICAL FIX (Nov 19, 2025): Frontend should NOT manually create messages!
            // The BACKEND already creates the proper message structure:
            // 1. Assistant message with tool_use blocks
            // 2. User message with tool_result blocks (line 2121 in combined_agent_worker.py)
            // 3. Assistant message with text response
            //
            // Frontend's job: Just save what the STREAMING worker already built
            // The conversation_history on the backend is the source of truth
            //
            // TODO: Instead of reconstructing messages here, we should:
            // - Let the backend send a 'conversation_update' event with the full conversation
            // - OR: Track each assistant/user message separately as events arrive
            //
            // For now, keep existing behavior (save text only) to avoid breaking changes
            // The real fix is on the backend (consecutive user message merging)
            ...
            if (fullResponse && fullResponse.trim().length > 0) {
                ...
                if (window.MessageStore) {
                    ...
                    console.log('✅ [MessageStore] Assistant text response added to conversation history');
                    console.log('[INFO] Backend conversation_history is the source of truth (includes tool_use/tool_result blocks)');
                } else {
                    console.error('[ERROR] MessageStore not available for saving assistant response');
                }
            } else {
                console.log('[WARN] No text response to add to history (thinking/tools only)');
            }
