/**
 * SURGICAL PATCH FOR streamingTwoRule.js
 * 
 * SAFE INTEGRATION: Adds SVG/CAD/LaTeX rendering without breaking Plotly/Mermaid
 * 
 * INSTRUCTIONS:
 * 1. Copy methods below into TwoRuleStreamProcessor class
 * 2. Update getStartDelimiter() to add 6 new types
 * 3. Add 2 new branches to renderVisualization()
 * 
 * ZERO BREAKING CHANGES - Uses existing .viz-content-area container structure
 */

// ============================================================================
// PART 1: UPDATE EXISTING DELIMITER METHODS (Lines ~1277, ~1310)
// ============================================================================

// FIND getStartDelimiter() and UPDATE to:
...
// FIND getEndDelimiter() and UPDATE to:
...
// ============================================================================
// PART 2: ADD NEW BRANCHES TO renderVisualization() (Line ~950)
// ============================================================================

// FIND renderVisualization() and ADD these branches AFTER the Mermaid block:
//
// ...existing Plotly block (lines 980-998)...
// ...existing Mermaid block (lines 1005-1015)...
//
// ✨ ADD THESE TWO BLOCKS:

else if (['svg', 'cad', 'schematic', 'blueprint', 'molecule'].includes(type)) {
    // Route to SVG renderer
    await this.renderSVGVisualization(type, content, targetContainer);
}
else if (type === 'latex') {
    // Route to LaTeX renderer
    await this.renderLatexVisualization(content, targetContainer);
}
else if (type === 'html') {
    // Route to HTML renderer (inline executable)
    await this.renderHTMLVisualization(content, targetContainer);
}
else if (type === 'chartjs') {
    // Route to Chart.js renderer
    await this.renderChartJSVisualization(content, targetContainer);
}
else if (type === 'apexcharts') {
    // Route to ApexCharts renderer
    await this.renderApexChartsVisualization(content, targetContainer);
}
else if (type === 'threejs') {
    // Route to Three.js renderer
    await this.renderThreeJSVisualization(content, targetContainer);
}
else if (type === 'gsap') {
    // Route to GSAP renderer
    await this.renderGSAPVisualization(content, targetContainer);
}
else if (type === 'lottie') {
    // Route to Lottie renderer
    await this.renderLottieVisualization(content, targetContainer);
}

// ...existing fallback (keep)...

// ============================================================================
// PART 3: ADD NEW METHODS (After renderVisualization(), ~line 1020)
// ============================================================================

/**
 * Render SVG-based visualizations (CAD, schematics, blueprints, molecules)
 * CRITICAL: Uses SAME .viz-content-area container as Plotly/Mermaid
 * 
 * @param {string} type - Visualization type (svg, cad, schematic, blueprint, molecule)
 * @param {string} svgCode - Raw SVG code from delimiter
 * @param {HTMLElement} container - The .viz-content-area element
 */
async renderSVGVisualization(type, svgCode, container) {
    console.log(`🎨 TWO-RULE: Rendering ${type} SVG into .viz-content-area`);
    ...
    try {
        ...
        // 6. Add control buttons (needs wrapper reference for popup)
        const controls = this.createSVGControls(sanitized, type, wrapper);
        wrapper.insertBefore(controls, wrapper.firstChild);
        
        // 7. ✅ CRITICAL: Clear container and insert (same as Plotly/Mermaid)
        container.innerHTML = '';
 ...
 * CRITICAL: Uses SAME .viz-content-area container as Plotly/Mermaid
 * 
 * @param {string} latexCode - LaTeX equation code
 * @param {HTMLElement} container - The .viz-content-area element
 */
async renderLatexVisualization(latexCode, container) {
    console.log('🔢 TWO-RULE: Rendering LaTeX equation into .viz-content-area');
    ...
    try {
        ...
        // 4. ✅ CRITICAL: Clear container and insert (same as Plotly/Mermaid)
        container.innerHTML = '';
        container.appendChild(wrapper);
 ...
 * CRITICAL: Uses SAME .viz-content-area container as Plotly/Mermaid
 * SECURITY: Sanitized but allows scripts for interactivity
 * 
 * @param {string} htmlCode - Raw HTML code with inline scripts
 * @param {HTMLElement} container - The .viz-content-area element
 */
async renderHTMLVisualization(htmlCode, container) {
    console.log('🌐 TWO-RULE: Rendering inline HTML into .viz-content-area');
    
    try {
        // 1. Create sandboxed wrapper with COMPLETE isolation from parent UI
        const wrapper = document.createElement('div');
        ...
        // 2. Create sandboxed iframe for isolated execution (FULL ISOLATION)
        ...
        iframe.style.cssText = `
            ...
            isolation: isolate;
        `;
        // ⚠️ CRITICAL: Sandbox prevents CSS/JS from affecting parent document
        iframe.sandbox = 'allow-scripts allow-same-origin';
        
        // 🔒 CRITICAL: Additional isolation - prevents style leakage
        iframe.setAttribute('referrerpolicy', 'no-referrer');
        iframe.setAttribute('loading', 'eager');
        
        // 3. Add expand button for popup view
        ...
        // 4. Build COMPLETELY isolated HTML document
...
<head>
    ...
    <style>
        /* CRITICAL: Full CSS reset to prevent parent style inheritance */
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* Prevent any global CSS leakage */
        * { all: revert; }
        body * { all: revert; }
    </style>
</head>
...
${htmlCode}
        ...
        // 5. Inject via srcdoc (MORE SECURE than write())
        ...
        // 7. Auto-resize iframe to content height (with safety delay)
        ...
        // 8. ✅ CRITICAL: Clear container COMPLETELY and insert wrapper
        container.innerHTML = '';
        container.appendChild(wrapper);
        
        console.log('🔒 HTML rendered in FULLY ISOLATED iframe (no UI impact)');
        
        console.log('✅ TWO-RULE: HTML rendered successfully in sandboxed iframe');
 ...
 * @param {HTMLElement} originalWrapper - Original wrapper element
 */
...
// ============================================================================
// PART 3B: CHART LIBRARY RENDERERS
// ============================================================================

/**
 * Render Chart.js visualization from JSON config
 * 
 * @param {string} configJSON - JSON configuration for Chart.js
 * @param {HTMLElement} container - The .viz-content-area element
 */
async renderChartJSVisualization(configJSON, container) {
    console.log('📊 TWO-RULE: Rendering Chart.js into .viz-content-area');
    
    try {
        // 1. Load Chart.js library if not already loaded
 ...
 * 
 * @param {string} configJSON - JSON configuration for ApexCharts
 * @param {HTMLElement} container - The .viz-content-area element
 */
async renderApexChartsVisualization(configJSON, container) {
    console.log('📈 TWO-RULE: Rendering ApexCharts into .viz-content-area');
    
    try {
        // 1. Load ApexCharts library if not already loaded
...
// ============================================================================
// PART 3C: 3D & ANIMATION RENDERERS
// ============================================================================

/**
 * Render Three.js 3D scene from JSON config
 * 
 * @param {string} configJSON - JSON configuration for Three.js scene
 * @param {HTMLElement} container - The .viz-content-area element
 */
async renderThreeJSVisualization(configJSON, container) {
    console.log('🎮 TWO-RULE: Rendering Three.js into .viz-content-area');
    
    try {
        // 1. Load Three.js library if not already loaded
        ...
        // 7. Build Three.js scene from config
 ...
 * @param {HTMLElement} container - Container div for renderer
 */
 ...
 * 
 * @param {string} configJSON - JSON configuration for GSAP
 * @param {HTMLElement} container - The .viz-content-area element
 */
async renderGSAPVisualization(configJSON, container) {
    console.log('✨ TWO-RULE: Rendering GSAP animation into .viz-content-area');
    
    try {
        // 1. Load GSAP library if not already loaded
 ...
 * 
 * @param {string} configJSON - JSON configuration for Lottie
 * @param {HTMLElement} container - The .viz-content-area element
 */
async renderLottieVisualization(configJSON, container) {
    console.log('🎬 TWO-RULE: Rendering Lottie animation into .viz-content-area');
    
    try {
        // 1. Load Lottie library if not already loaded
...
/**
 * Generic chart popup handler (works for Chart.js, ApexCharts, Three.js)
 * 
 * @param {string} type - Chart type
 * @param {string} configJSON - JSON configuration
 * @param {HTMLElement} originalWrapper - Original wrapper
 */
openChartPopup(type, configJSON, originalWrapper) {
    ...
    // Content - re-render chart at larger size
    const contentDiv = document.createElement('div');
    contentDiv.className = 'viz-popup-content';
    
    // Re-render at larger size based on type
    if (type === 'chartjs') {
...
// ============================================================================
// PART 4: HELPER METHODS - Security (Add after renderHTMLVisualization)
// ============================================================================

/**
 * Sanitize SVG code to prevent XSS attacks
 * Removes: <script>, <iframe>, <object>, event handlers, javascript: protocols
 * 
...
sanitizeSVG(svgCode) {
    ...
    dangerousTags.forEach(tag => {
        // Remove both opening/closing tags and their content
    ...
    // 4. Remove data: URLs (potential XSS vector)
...
/**
 * Enhance SVG element with responsive attributes and accessibility
 * 
 * @param {SVGElement} svgElement - Parsed SVG DOM element
 * @param {string} type - Visualization type for labeling
 */
enhanceSVGElement(svgElement, type) {
    // 1. Ensure viewBox exists for responsive scaling
    ...
    // 2. Remove fixed dimensions to make responsive
    ...
    console.log(`✨ Enhanced ${type} SVG: responsive + accessible`);
}

// ============================================================================
// PART 5: HELPER METHODS - UI Controls
// ============================================================================

/**
 * Create control buttons for SVG (expand to popup)
 * 
 * @param {string} svgContent - Original SVG code
 * @param {string} type - Visualization type
 * @param {HTMLElement} wrapper - The wrapper element
 * @returns {HTMLElement} Control bar with buttons
 */
...
/**
 * Open SVG/LaTeX in draggable popup modal
 * 
 * @param {string} content - SVG or LaTeX content
 * @param {string} type - Visualization type
 * @param {HTMLElement} originalWrapper - Original wrapper element
 */
openSVGPopup(content, type, originalWrapper) {
    ...
    // Create new popup container (don't clone to avoid event listener issues)
    const popup = document.createElement('div');
    ...
    // Copy raw code button (</> icon)
    const copyRawBtn = document.createElement('button');
    copyRawBtn.className = 'viz-popup-header-btn';
    copyRawBtn.innerHTML = '&lt;/&gt;';
    copyRawBtn.title = 'Copy raw code';
    copyRawBtn.onclick = () => {
        try {
            navigator.clipboard.writeText(content);
            this.showNotification('✅ Raw code copied!', 'success');
            copyRawBtn.innerHTML = '<i class="fas fa-check"></i>';
            setTimeout(() => copyRawBtn.innerHTML = '&lt;/&gt;', 1500);
        } catch (error) {
            this.showNotification('❌ Copy failed', 'error');
        }
    };
    ...
    // Assemble header
    headerControls.appendChild(copyBtn);
    headerControls.appendChild(copyRawBtn);
    ...
    // Re-parse and render SVG from content (fixes schematic rendering issue)
    try {
        const parser = new DOMParser();
...
// ============================================================================
// PART 6: HELPER METHODS - Theming & Styling
// ============================================================================

/**
 * Get type-specific wrapper styles for professional appearance
 * 
 * @param {string} type - Visualization type
...
// ============================================================================
// PART 7: HELPER METHODS - Utilities
// ============================================================================
...
/**
 * Dynamically load KaTeX library for LaTeX rendering
 * 
 * @returns {Promise} Resolves when KaTeX is loaded
 */
async loadKaTeX() {
    return new Promise((resolve, reject) => {
        ...
        if (!document.querySelector('script[src*="katex.min.js"]')) {
            ...
            script.onerror = (error) => {
                ...
                reject(new Error('Failed to load KaTeX library'));
            };
...
/**
 * Dynamically load JavaScript library from CDN
 * 
...
async loadScript(url) {
    return new Promise((resolve, reject) => {
        ...
        script.src = url;
        script.onload = () => {
            console.log(`✅ Library loaded successfully: ${url}`);
            resolve();
        };
        script.onerror = (error) => {
...
loadGoogleFont(fontFamily) {
    ...
    const href = `https://fonts.googleapis.com/css2?family=${safeName}:wght@400;700&display=swap`;
...
// ============================================================================
// END OF SURGICAL PATCH
// ============================================================================

/**
 * INTEGRATION SUMMARY:
 * 
 * Files Modified: 1 (streamingTwoRule.js)
 * Lines Added: ~330
 * Methods Modified: 2 (getStartDelimiter, getEndDelimiter)
 * Methods Added: 10 new methods
 * Breaking Changes: ZERO
 * Risk Level: LOW 🟢
 * 
 * New Visualization Types: 6
 *   - SVG, CAD, SCHEMATIC, BLUEPRINT (SVG-based)
 *   - LATEX (KaTeX-based)
 *   - MOLECULE (SVG-based)
 * 
 * Existing Functionality: 100% Preserved
 *   - Plotly charts work exactly as before
 *   - Mermaid diagrams work exactly as before
 *   - Loading indicators work for all types
 *   - Container structure unchanged (.viz-content-area)
 * 
 * Testing Required:
 *   1. Plotly chart (verify no regression)
 *   2. Mermaid diagram (verify no regression)
 *   3. New SVG visualization (verify rendering)
 *   4. New LaTeX equation (verify KaTeX loads)
 *   5. Download button (verify SVG export)
 *   6. Copy button (verify clipboard)
 */
