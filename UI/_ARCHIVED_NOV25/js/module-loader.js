...
async function initializeModuleSystem() {
    ...
    while (!window.ModuleManager && (Date.now() - startTime) < maxWait) {
        console.log('💜💜 [MODULES] ⏳ Waiting for ModuleManager class...');
        await new Promise(resolve => setTimeout(resolve, 100));
    }

    if (!window.ModuleManager) {
        console.error('💜💜 [MODULES] ❌ ModuleManager not found after timeout');
        console.log('💜💜 [MODULES] Available window properties:', Object.keys(window).filter(k => k.includes('Module')));
        return;
    }
    ...
    try {
        ...
        // ✅ CRITICAL FIX: await the async initialize() call
        ...
        if (initialized) {
            ...
            console.log('💜💜 [MODULES] ✅ Module system ready');
        } else {
            console.error('💜💜 [MODULES] ❌ ModuleManager failed to initialize - DOM elements missing?');
        }
    } catch (error) {
        console.error('💜💜 [MODULES] ❌ Module system initialization error:', error);
...
let moduleSystemInitPromise = null;
...
async function safeInitializeModuleSystem(forceLoad = false) {
    ...
    if (actuallyInitialized && !forceLoad) {
        console.log('💜💜 [MODULES] ⏭️ Module system already initialized with', window.ModuleManager.getModules().length, 'modules');
        return;
    }

    // If already initializing, WAIT for it instead of aborting
    if (moduleSystemInitializing && moduleSystemInitPromise) {
        console.log('💜💜 [MODULES] ⏳ Already initializing - WAITING for completion...');
        await moduleSystemInitPromise;
        console.log('💜💜 [MODULES] ✅ Initialization complete, continuing...');
        return;
    }

    // Reset flag if it was set but modules weren't loaded
    if (moduleSystemInitialized && window.ModuleManager?.getModules().length === 0) {
        console.log('💜💜 [MODULES] ⚠️ Flag was set but no modules loaded, resetting...');
    ...
    moduleSystemInitPromise = (async () => {
        try {
            ...
            moduleSystemInitializing = false;
            console.log('💜💜 [MODULES] ✅ Initialization process complete | Success:', moduleSystemInitialized);
        }
    })();
