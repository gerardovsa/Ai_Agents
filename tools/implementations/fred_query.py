"""
Fred (InHouse) Database Query & Search Tools
============================================

Two powerful tools for querying the Fred InHouse print shop database (SQL Server):

1. fred_execute_query() - Execute custom SQL queries on Fred database
2. fred_search_database() - Search across Fred database for text/keywords
...
def fred_search_database(
    ...
    Examples:
        ...
        fred_search_database(search_text='INV-2025-')
...
__tools__ = {
    'fred_execute_query': {
        ...
        'examples': [
            "fred_execute_query('SELECT TOP 10 * FROM Orders WHERE ClientName LIKE ?', ['%Printing%'])",
            "fred_execute_query('SELECT * FROM JobTickets WHERE OrderID = ?', [56230])"
        ]
    },
    
    'fred_search_database': {
        'function': fred_search_database,
        ...
        'examples': [
            "fred_search_database('CJ King Printing')",
            "fred_search_database('%business cards%', tables=['JobTickets'])"
        ]
    }
}
